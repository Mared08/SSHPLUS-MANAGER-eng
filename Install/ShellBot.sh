...
#						and functions that allow the developer:
#
...
#
#						ShellBot maintains the standard for nomenclature of registered methods of
#						Original API (Telegram), as well as its fields and values. The methods
#						require parameters and arguments for the call and execution. Parameters
#						required return an error message if the argument is omitted.
#					
#	NOTE:				Developed in the Shell Script language, using the 
...
if ! awk 'BEGIN { exit ARGV[1] < 4.3 }' ${BASH_VERSINFO[0]}.${BASH_VERSINFO[1]}; then
	echo "${BASH_SOURCE:-${0##*/}}: error: requires the command interpreter 'bash 4.3' or superior." 1>&2
	exit 1
fi
...
# Disables file name expansion (globbing).
set -f
...
readonly _ERR_SERVICE_SYSTEMD_NOT_FOUND_='error when activating: the system does not support service management "systemd".'
...
readonly _ERR_ARG_='function not found: the specified identifier is invalid or does not exist.'
...
readonly _ERR_CONNECTION_='connection failure: unable to connect to Telegram.'
...
MessageError()
{
	...
	# Gets the index of the function in the calling hierarchy.
	[[ ${FUNCNAME[1]} == CheckArgType ]] && i=2 || i=1
	
	# Reads the type of occurrence.
	...
	# Terminate script / thread in case of internal error, otherwise it returns 1
	${assert:-false} && exit 1 || return 1
}

CheckArgType()
{
	# CheckArgType receives data from the calling function and checks
	# The data received with the type supported by the parameter.
	# '0' is returned for success, otherwise a message
	# error is returned and the script / thread terminates with status '1'.
	case $1 in
...
CreateUnitService()
{
	...
	# 'service' mode requires the process management system 'systemd'
	# be present so that the Unit target is linked to the service.
...
ShellBot.init()
{
				...
				-t|--token)
	   			...
	   			-m|--monitor)
					...
					monitor=true
	   				...
	   				;;
				-f|--flush)
					# Sets the FLAG flush to the 'ShellBot.getUpdates' method. If enabled, it causes
					# the method obtains only available updates, ignoring the extraction of
    ...
    # It does not require parameters. Returns basic information about the bot in the form of a User object.
    ShellBot.getMe()
    {
    	# Calls the getMe method by passing the API address, followed by the method name.
    ...
    # SHELLBOT (FUNCTION)
	# Initializes the functions for calling the Telegram API methods.
    ...
    ShellBot.InlineKeyboardButton()
    {
    	...
    	do
    		case $1 in
    			-b|--button)
    				# Pointer that receives the "button" address with the definitions
    				# the configuration of the inserted button.
    ...
    ShellBot.InlineKeyboardMarkup()
    {
    	...
    	# Build object structure + keyboard array, set values ​​and save settings.
    ...
    # Creates an object that represents a custom keyboard with response options
    ShellBot.ReplyKeyboardMarkup()
    {
    	...
    	# Transforms function parameters into positional parameters
    	#
    	# Example:
    	...
    	# Awaits reading of parameters
    	while :
    	do
    		# Read the first position parameter "$1"; If it is a valid parameter,
    		# saves the value of the argument in position '$2' and shifts two positions to the left (shift 2); Repeat the process
    	...
    	# Prints an error message if the mandatory parameter is omitted.
    	[[ $__button ]] || MessageError API "$_ERR_PARAM_REQUIRED_" "[-b, --button]"
    ...
    ShellBot.sendMessage()
    {
    	...
    	# and values ​​are passed on the form and read by the method. The method return is redirected to the file 'update.Json'.
